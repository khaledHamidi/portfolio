...
(function() {
  ...
  class ScrollAnimations {
    ...
    init() {
      ...
      document.querySelectorAll('section, .card, .grid > a, article').forEach(el => {
        ...
        // Immediately show elements already visible on page load
  ...
  function initEffects() {
    // Always initialize particle system on every page
    const particleSystemInstance = new ParticleSystem();
    
    // Initialize TiltCards for hero section
    const heroSection = document.querySelector('.hero-section');
    if (heroSection) {
      setTimeout(() => { new TiltCards(); }, 500);
    }
